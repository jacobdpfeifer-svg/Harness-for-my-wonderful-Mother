...
wp-price --db "$DB" init-db
wp-price --db "$DB" ingest-csv \
...
python3 scripts/load_market_snapshots.py "$DB" data/dec2023/market_snapshots.csv

AS_OF=2023-11-15
wp-price --db "$DB" signals run --collector resort --market grand_home \
  --as-of "$AS_OF" --fixture "$FIX/winter_park_resort.json"
wp-price --db "$DB" signals run --collector cdot --market grand_home \
  --as-of "$AS_OF" --fixture "$FIX/cdot_closure.json"
wp-price --db "$DB" signals promote

wp-price --db "$DB" signals rescore --from 2023-11-01 --to 2023-11-28
wp-price --db "$DB" recommend --from 2023-12-01 --to 2023-12-31 \
