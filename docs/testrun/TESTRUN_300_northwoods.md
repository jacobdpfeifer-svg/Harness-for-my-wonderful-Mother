...
- Do not run **unscoped** `recommend`, `audit`, `export`, or `push`. Every invocation must
  carry `--property overlook_ridge`. Never run `push` in this diagnostic.
...
- Do not use Guesty rates as an input, anchor, validation target, or comp source during the
  blind phase.
- Use an isolated disposable database (`--db /tmp/testrun_overlook_ridge.db`) or a clearly
...
- **Suggestions only: do not implement fixes.** The only file you create is your report,
  `docs/reports/TESTRUN_300_northwoods.md`.
...
- run relevant signals + `wp-price health --property overlook_ridge`;
...
```bash
wp-price recommend \
  --property overlook_ridge \
  --from 2026-09-24 \
  --to 2027-09-23 \
  --limit 500 \
  --technical \
  --db /tmp/testrun_overlook_ridge.db
```

Then export the complete set, scoped:
`wp-price export --property overlook_ridge --from 2026-09-24 --to 2027-09-23 -o data/exports/testrun_300.csv --db /tmp/testrun_overlook_ridge.db`.
...
(this property historically threw the worst **low-confidence-ceiling outliers** — e.g. a June
night Guesty sold at $228 while the engine wanted $1,060 off a 45%-confidence `seasonal_anchor`
ceiling with no comps); prices affected by missing pacing; any price that fails the
independent audit due to rounding; whether elasticity defaults appear to suppress prices; and
whether the ceiling is defensible for this property and date.
...
## Phase 5 — final report → `docs/reports/TESTRUN_300_northwoods.md`
...
## Property-specific focus — 300 Northwoods (twin-consistency)
This is the other "twin" (near-identical 5bd to `summit_haus` / 312 Northwoods). You must
specifically investigate: **whether the two Northwoods properties are being treated as genuine
twins** — whether they receive materially different recommendations, and whether any such
differences are **supported by evidence rather than accidental property-specific data gaps**
(e.g. one having more own-history rows, a different comp match, or a different ceiling method).
Report any unjustified divergence for the cross-property synthesis.
